...
export async function POST(request: Request) {
  try {
    ...
    console.log("API POST request received with body:", body);
    ...
    if (!apiKey) {
      ...
      console.log("NO API KEY - Using default roadmap");
      return NextResponse.json({
        units: getDefaultRoadmap(body.roleId, body.roleName)
      });
    }
...

{
  ...
  "roadmap": [
    ...
    const units: RoadmapUnit[] = parsedResponse.roadmap.map((unit: any) => ({
    ...
    console.log("Successfully parsed and transformed AI response");
    
    return NextResponse.json({ units });
    
  } catch (error: any) {
    console.error("=== ERROR IN CAREER QUEST ROADMAP API ===");
    ...
    return NextResponse.json(
      {
        units: getDefaultRoadmap("default", "General Role")
      },
      { status: 200 }
    );
  }
